...
def to_text(value):
    ...
    try:
        ...
        if number.is_integer() and "e" not in value.lower() and "." in value:
...
def make_extra(row, used):
    return {key: value for key, value in row.items() if key not in used and clean(value) is not None}


def read_csv_table(path, sheet_key):
    header_index = HEADER_ROWS[sheet_key] - 1
        ...
        for index, values in enumerate(rows):
            ...
            if index == header_index:
                while values and values[-1] is None:
                    values.pop()
                headers = [value or f"col_{i + 1}" for i, value in enumerate(values)]
                continue
            if index <= header_index or not headers:
                continue
...
def parse_sb1(row):
    used = {"Codigo", "Descricao", "Descricao NF", "Tipo", "Unidade", "Armazem Pad.", "Grupo", "Pos.IPI/NCM", "Aliq. ICMS", "Aliq. IPI", "Preco Venda", "Ult. Preco", "Ult. Compra"}
...
def parse_sd1(row):
    used = {"Filial", "Item NF", "Produto", "Unidade", "Quantidade", "Armazem", "Vlr.Unitario", "Custo Moeda1", "Vlr.Total", "Forn/Cliente", "Loja", "Documento", "DT Emissao", "DT Digitacao", "Serie", "Grupo", "Tipo Produto"}
    code = to_text(row.get("Produto"))
    if not code:
        return None
    return {
...
def main():
    parser = argparse.ArgumentParser(description="Gera o seed JSON completo a partir dos CSVs exportados do XLSB.")
    parser.add_argument("--csv-dir", default="work/extracted-csv")
    parser.add_argument("--output", default="backend/prisma/seed-data/sample.json")
    ...
    for key in ["sb1", "sb2", "sd1", "sa1", "sa2"]:
        ...
        for row in read_csv_table(path, key):
