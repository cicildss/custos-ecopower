      ...
      <div className="flex flex-col gap-3 border-b border-line px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-600">
          <button
            ...
            className="grid h-9 w-9 place-items-center rounded-md border border-line bg-white disabled:opacity-40"
          >
        ...
        <table className="w-full min-w-[960px] text-sm">
          ...
          <tbody>
            {data.rows.map((row, index) => (
              <tr key={`${row.d1_doc}-${row.d1_serie}-${index}`} className="border-t border-line">
                ...
                <td className="px-4 py-3">
                  ...
                  <span className="block max-w-56 truncate text-slate-600">{row.fornecedor.nome ?? "Fornecedor não cadastrado"}</span>
                </td>
